...
- `index.html` — Complete standalone app (React via CDN, pre-compiled, no build step)
- `hopfion_v32.jsx` — Raw React component source for editing (requires Babel to recompile into index.html)
- `README.md` — This file

## Editing

To modify the calculator, edit `hopfion_v32.jsx` and recompile:

```bash
npm install -g @babel/cli @babel/core @babel/plugin-transform-react-jsx
babel hopfion_v32.jsx --plugins='@babel/plugin-transform-react-jsx' -o app.js
```

Then paste the compiled output into `index.html` between the `<script>` tags.
